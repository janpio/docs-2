export const description = 'Comparison to SST'
...
3. **Infrastructure Provisioning:** Nitric uses Pulumi by default for provisioning cloud resources, but also allows the use of [custom providers](/reference/providers/custom/building-custom-provider). SST uses the AWS CDK, which uses AWS CloudFormation for provisioning.
